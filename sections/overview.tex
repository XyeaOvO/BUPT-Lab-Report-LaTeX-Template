...
\subsubsection{排版规范与风格}
\begin{itemize}
  \item 统一使用 `booktabs` 处理表格线条，避免竖线；使用 `threeparttable` 添加表注。
  \item 代码块使用 `maccode` 环境，并在标题中注明文件名；算法用 `algorithm2e` 自动编号。
  \item 重要提醒用彩色盒子高亮，定义与定理使用统一样式，增强可读性。
\end{itemize}
