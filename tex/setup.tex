...
\else
  ...
  % Mono: 优先常见跨平台等宽字体，尽量避免无效字体探测噪声
  \SetFontWithFallback{\setmonofont}{
    Scale=MatchLowercase
  }{JetBrains Mono, DejaVu Sans Mono, Latin Modern Mono, Courier New, Consolas}
...
\ifXeTeX
    ...
    \SetFontWithFallback{\setCJKmonofont}{
      AutoFakeBold=2
    }{Noto Sans Mono CJK SC, Source Han Sans Mono SC, SimHei, 黑体}
...
\newcommand{\DrawLogo}{%
  \IfFileExists{\LogoFile}{%
    \raisebox{-0.23cm}{\includegraphics[height=0.75cm]{\LogoFile}}%
  }{}%
}
...
% --- 页眉设置 ---
% 左侧放 Logo（如果存在），视觉上更平衡
\fancyhead[L]{\DrawLogo}
% 右侧放 课程名 | 报告标题（层级高）
\fancyhead[R]{%
    \small\sffamily\color{GrayText}%
    \CourseName\ \quad\textcolor{BUPTBlue}{|}\quad \ReportTitle%
}
...
% --- 页脚设置 ---
% 左侧放作者名（次要信息）
\fancyfoot[L]{\small\sffamily\color{GrayText}\AuthorName}

% 右侧放页码：纯数字，蓝色加粗，不带框，更加清爽
\fancyfoot[R]{%
  \small\sffamily\color{GrayText}%
  \ifMainMatter
    \textbf{\color{BUPTBlue}\thepage}\ / \pageref{LastPage}%
  \fi
}

% 取消页脚分割线（让页面下方留白更多，不压抑）
...
\ifUseMinted
  % ==================== Minted 引擎 ====================
  % latexmk 使用 -output-directory=build 时，minted 需要同步 outputdir
